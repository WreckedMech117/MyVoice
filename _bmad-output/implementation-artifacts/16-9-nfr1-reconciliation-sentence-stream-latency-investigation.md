...
**Modified:**
- `scripts/validate_streaming_default.py` (~280 net new lines: PhaseProfile dataclass, _profile_phases context manager, model-load patch, four new argparse flags `--profile-phases` / `--quality-tier` / `--stratified-sample` / `--output-csv-name`, stratified-sample selector, per-class per-phase aggregate printer; existing Story 16.7 invocation paths unchanged when none of the new flags are set)
- `_bmad-output/planning-artifacts/architecture-optimization-pass.md` (NFR1 cell inline pointer at line 802 + new prose sub-section at line 819)
...
- `_bmad-output/implementation-artifacts/16-9-nfr1-reconciliation-report.md` (~330 lines, 8 sections per AC #6)
- `_bmad-output/implementation-artifacts/16-9-correct-course-nfr1-revision.md` (~110 lines, stakeholder routing artifact for outcome (c) sign-off)
