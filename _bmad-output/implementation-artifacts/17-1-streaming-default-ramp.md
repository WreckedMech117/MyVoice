...
>
> (a) **Recruit ≥2 additional listeners (L2, L3) and prepare audition packets.** Listener selection is friends / family / collaborators per the LISTENING-INSTRUCTIONS.md framing — the audition is not a corporate UAT. Each listener receives the fixture directory + LISTENING-INSTRUCTIONS.md + their listener ID (L2 or L3) + a results-CSV template (or instructions to send observations as plain text / spreadsheet for the maintainer to fold into the canonical CSV). Listener identity is anonymized in the committed artifact; Commander retains a private mapping for follow-up correspondence if a defect needs reproducing. Listeners conduct the audition on their own playback hardware (headphones if available; not a single shared listening environment) — this is the LISTENING-INSTRUCTIONS.md protocol verbatim and reflects realistic Discord-call usage.
>
...
**When** each listener accepts the audition packet (fixture directory + LISTENING-INSTRUCTIONS.md + listener-ID assignment + results-CSV template or plain-text instructions)
...
**And** each listener records 10 rows in the canonical CSV schema
**And** each listener's identity is anonymized in the committed artifact (use `L2`, `L3`); Commander retains a private mapping for follow-up correspondence if a defect needs reproducing
