...
**Then** the report contains 7 sections in this order:
  1. **Executive summary** — one paragraph + a single recommendation line: `RECOMMENDATION: <PASS-FLIP / PASS-LEAVE / FAIL-TIGHTEN-PARAMS / FAIL-D8-FOLLOWUP / DEFER-PERCEPTUAL>` followed by the one-line code change it implies (e.g., `Code change: none — TRUE_STREAM already-default on CUDA per Story 16.6` OR `Code change: edit codec_token_streamer.py:46 DEFAULT_CHUNK_SIZE from 25 to 35 and re-run harness` OR `Code change: defer; add D-8 follow-up story for dedicated torch.cuda.Stream`)
...
**Given** the report's executive summary recommendation is one of `PASS-FLIP / PASS-LEAVE / FAIL-TIGHTEN-PARAMS / FAIL-D8-FOLLOWUP / DEFER-PERCEPTUAL`
...
NEW (committed via `git add -f` at story completion per AC #7):
- `scripts/validate_streaming_default.py` — GPU + CPU latency measurement harness (~625 lines incl. docstring/comments)
- `scripts/build_streaming_perceptual_ab_fixture.py` — Perceptual A/B fixture builder + per-listener truth-table + LISTENING-INSTRUCTIONS.md generator (~390 lines)
...
PENDING (produced by Tasks 4–6 user execution):
- `_bmad-output/implementation-artifacts/16-7-gpu-latency-measurements.csv` — Task 4 output (TRUE_STREAM on RTX 5090)
- `_bmad-output/implementation-artifacts/16-7-gpu-sentence_stream-comparison.csv` — Task 4 apples-to-apples GPU run
- `_bmad-output/implementation-artifacts/16-7-cpu-baseline-measurements.csv` — Task 5 output (SENTENCE_STREAM on CPU)
- `_bmad-output/implementation-artifacts/16-7-perceptual-fixtures/` — Task 6 directory (paired WAVs + `_perlistener_truthtable.json` + `LISTENING-INSTRUCTIONS.md`)
- `_bmad-output/implementation-artifacts/16-7-perceptual-ab-results.csv` — Task 6 audition output
- `_bmad-output/implementation-artifacts/16-7-streaming-validation-report.md` — Task 7 final report

MODIFIED (post-empirical-run scope expansion):
- `src/myvoice/services/qwen_tts_service.py` — `_generate_true_stream` empty-chunk guard (lines 2845–2861, ~17 lines). Was "no edits to `src/myvoice/`" per original scope; expanded under user direction to fix the Sev-1 silent-audio bug surfaced by the first empirical run (see Change Log entry 2026-05-07 second row).
- `tests/integration/test_streaming_tts_smoke.py` — new `TestSilentTalkerSurfacesAsFailure` class (2 tests, ~150 lines) at end of file; both marked `qt_no_exception_capture` because the worker's downstream `finalize()` Qt-slot exception is benign in production (logged) but would otherwise fail pytest-qt's strict mode.
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — `16-7-empirical-validation-gates-for-streaming-default: ready-for-dev → in-progress` (workflow Step 4); will flip to `review` at Task 8.1.
- `_bmad-output/implementation-artifacts/16-7-empirical-validation-gates-for-streaming-default.md` — this story file; Tasks/Subtasks 1.1–3.7 checked, File List + Completion Notes filled, Change Log appended.
