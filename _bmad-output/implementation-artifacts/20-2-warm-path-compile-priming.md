...
(`QwenTTSRequest.suppress_audio_output`) resolved through a single accessor
(`QwenTTSService._audio_chunk_sink(request)`) that every chunk-emit site now
calls. Rejected detach-and-restore of `_audio_chunk_ready_callback` and a
service-level suppression boolean: both are scoped to a *window of time*, so
both would silence a user generation dispatched while the ~4.4 s priming is in
flight — AC #3's named failure mode. The per-request flag travels with the
generation it describes, so concurrency is a non-issue and startup ordering is
irrelevant. To keep it hard to get wrong from a future caller's perspective,
a source-invariant test fails the build if `self._audio_chunk_ready_callback`
is read anywhere except its declaration, its setter, and `_audio_chunk_sink`.
...
**Regression sweep.** Touched surface clean: `tests/unit/services` +
`tests/unit/observability` + `tests/unit/models` = 896 passed;
`tests/integration` 174 passed. All failures across the wider tree
(4 integration, 7 services/utils, 7 ui, 30 unit/ui) verified **pre-existing**
by stashing the source change and re-running — identical failure sets.
19 tests directly cover this story (9 new + 10 in the updated warmup file);
a mutation check (suppression removed) fails 4 of the 9 new tests, so the
...
- `tests/unit/services/test_qwen_tts_service_compile_warmup.py` — MODIFIED
  ...
  added the no-model-registry fast-exit row; 7 → 10 tests)
...
- `_bmad-output/implementation-artifacts/20-2-capture-long.sh`,
  `20-2-capture-short.sh`, `20-2-aggregate.py` — NEW
