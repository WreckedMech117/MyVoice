...
- **Listener-recruitment caveat documented.** L2 / L3 sessions were in-person walkthroughs with Commander as scribe rather than independent audits on diverse playback hardware. The walkthrough format preserved per-utterance per-listener structured defect labeling (the architectural defense the N≥3 rule was designed to provide; the helper's controlled-vocabulary input gates enforced the protocol). The walkthrough caveat is disclosed in the architecture amendment's H4 sub-section "Methodology footnote" paragraph; reviewers can consider it when interpreting the verdict's confidence.
...
- **Methodology disclosure:** L2 / L3 audition was conducted as in-person walkthroughs with Commander as scribe (not independent listener sessions on diverse playback hardware). Commander accepts this as a documented limitation worth disclosing in the architecture amendment but not gate-blocking; the per-utterance per-listener structured defect labeling (the architectural defense the N≥3 rule was designed to provide) was preserved by the helper's controlled-vocabulary input gates and the listeners' independent answer recording. See the architecture amendment's H4 sub-section "Methodology footnote" paragraph for the canonical record.
...
- Audition helper (working file, not committed): `_bmad-output/implementation-artifacts/17-1-l1-audition-helper.py` (Windows `winsound`-based audition driver; reads truth-table, plays trial-A/trial-B blind, validates inputs against the controlled vocabulary, appends rows incrementally to the canonical CSV).
