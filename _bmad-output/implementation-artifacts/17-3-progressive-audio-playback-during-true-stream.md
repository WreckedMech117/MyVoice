...
- [x] **Task 8 — Sprint-status finalization + epic-17 closure evaluation**
  - [x] 8.1 Sprint-status: `17-3-progressive-audio-playback-during-true-stream: ready-for-dev → in-progress` at story execution start (2026-05-08); `→ review` at story closure (2026-05-09).
  - [ ] 8.2 Code-review pass via `/bmad-bmm-code-review` — pending. Anticipated review surface (per Dev Notes "Previous-story intelligence"): callback-emission edge cases (zero-length chunks, sample-rate mismatches), event-loop scheduling under thread-safety constraints, `_progressive_playback_active` race conditions on rapid-fire-cancel flows, `stop_streaming_session()` idempotence on double-call. The "consume-once" flag-clear deviation from AC #2 step 4 is the most likely review surface; rationale documented inline in `_handle_progressive_chunk_async` docstring.
...
**Modified:**
- `src/myvoice/services/qwen_tts_service.py` — Task 1 (~40 LOC delta in `_wrapped_post`).
- `src/myvoice/app.py` — Task 2 + 3 + 4 + 5 (~140 LOC delta total: slots, callbacks, dispatch-skip, cancel-extension).
